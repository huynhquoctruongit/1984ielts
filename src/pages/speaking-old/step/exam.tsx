...
export const Exam = ({ quiz, isReady }: any) => {
  ...
  const onSubmit = async () => {
    ...
    if (typeof (audioUrl) == "object") {
      ...
      if (filePosts?.length > 0) {
        console.log(filePosts,'filePosts');
        const params: any = [];
  ...
  return (
    <div className="lg:p-0 p-[20px] mb-[20px]">
      ...
      <div className="max-w-[800px] m-auto text-left lg:mt-[50px] zoomin-content overflow-y-hidden">
        {part?.status == false ? (
          <div>
              ...
              {(isNextPart && urlAudio && sizeInBytes && (
                <div className="bg-white w-full m-auto bg-neu7 lg:px-[35px] px-[12px] py-[22px] rounded-[20px]">
                  ...
                  <div id="countdown" className={`flex justify-center ${isRec ? "pulse" : ""}`}>
                    <CountdownCircleTimer size={90} strokeWidth={6} duration={timeDownPart} isPlaying={isRec} colors={brand ? "#164675" : "#F99D1C"}>
                      {({ remainingTime }) => <MicroIcon className="fill-primary1" fill={brand ? "#164675" : "#F99D1C"} />}
