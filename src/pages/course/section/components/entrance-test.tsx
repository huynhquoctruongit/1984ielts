...
const ButtonEntranceTest = ({ classUser }) => {
  ...
  const [open, setOpen] = useLocalStorage(`open-entrance-${classUser.id}-${profile.id}`, false);
  const onSubmitEntranceTest = async () => {
    AxiosAPI.put(`/v1/e-learning/user/class/${classUser.id}/entrance-test/completed`);
    setOpen(false);
  };
  if (classUser.is_entrance_test !== true) return null;

  return (
    <div className="button-entrance-test">
      <div
        onClick={() => setOpen(true)}
        className="bg-neu4 py-2 text-center w-fit px-6 mx-auto rounded-full mt-10 cursor-pointer hover:bg-neu3 text-neu1 body3 duration-200 shadow-sm"
      >
        Đã hoàn thành
      </div>
