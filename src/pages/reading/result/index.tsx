...
const Result = ({ getLayout, classUser, isPayment, openPayment }: any) => {
  ...
  var correct = 0;
  const numberCorrect: any = listParts?.map((item: any) => {
  ...
  return (
    <div className="content-exam h-full">
      <HeaderUI isPayment={isPayment} openPayment={openPayment} classUser={classUser} part="Reading" title={quiz?.title} />
      <div className="max-w-[700px] m-auto p-[20px] zoomin-content overflow-y-hidden">
        ...
        <Link to={`${window.location.pathname.replace("/result", "") + window.location.search}&type=review`}>
