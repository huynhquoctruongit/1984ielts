...
const Reading = ({ indexPart, children }: any) => {
    ...
    useEffect(() => {
        setTimeout(() => {
            const content: any = document.getElementById("render-ui-question");
            if (content) {
                if (isMobile) {
                ...
                const openPopupUI = async (event) => {
                    ...
                    if (elm?.toString()) {
                        ...
                        event.clientX < 100 ? (plusWidth = 30) : 100;
                        setTimeout(() => {
                            var getTool: any = document.querySelector(".popup-selected") as any;
                            if (getTool) {
                                elm.toString().length
                                    ? ((getTool.style.left = event.clientX - plusWidth + "px"), (getTool.style.top = event.clientY - plusHeight + "px"), getTool.classList.add("active"))
                                    : null;
                            }
                        }, 100);
                    }
    ...
    const PopupHightlight = () => {
        return (
            <div
                ...
                id="popup-selected"
                className="popup-selected-hightlight w-fit fixed border-[0.5px] border-gray text-white bg-neu1 z-[1111] rounded-[4px] p-[10px]"
            >
                <div
    ...
    const Popup = () => {
        return (
            <div
                ...
                id="popup-selected"
                ref={refPopup}
                className="popup-selected w-fit fixed border-[0.5px] border-gray bg-neu1 text-white z-[1111] rounded-[4px] p-[10px]"
            >
    ...
    const Note = () => {
        ...
        setTimeout(() => {
            ...
            if (getTool) {
                var newLeft = oRect.left <= 120 ? 120 : oRect.left;
                (getTool.style.left = newLeft + oRect.width / 2 - 70 + "px"), (getTool.style.top = plusHeight + "px"), getTool.classList.add("active");
            }
        }, 100);
        let value = "";
                        ...
                        <div className="relative">
                            ...
                            {isTooltip && (
                                <div className="absolute mt-[6px] w-[100px] bg-white p-[12px] rounded-[8px] border-[1px] border-gray">
                                    ...
                                    <p className="cursor-pointer caption hover:text-primary1" onClick={() => onAction("delete")}>
                                        Delete
                                    </p>
                                </div>
                            )}
    ...
    return (
        <div>
            <div className="relative px-4" id="screen-question">
                {isHightlight?.status && <PopupHightlight></PopupHightlight>}
                {isPopup && <Popup></Popup>}
                {isNote.status && <Note></Note>}
                <div className="min-h-full">
                    <div id="render-ui-question" className={`relative ui-part-${indexPart}`}>
                        {children}
                    </div>
                </div>
            </div>
