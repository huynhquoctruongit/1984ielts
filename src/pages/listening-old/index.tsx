...
import amplitude, { amplitudeSendTrack, getTypeByUrl } from "@/services/amplitude";
...
const Reading = ({ getLayout }: any) => {
  ...
  const onSubmit = async () => {
    if (!isWaitSubmit) {
      setWaitSubmit(true)
      Object.keys(listAnswer).forEach(function (key, index) {
        listAnswer[key].map((elm: any) => {
  ...
  const Note = () => {
    ...
    return (
      <div
        ...
        style={{ boxShadow: "0px 0px 15px 0px rgb(0 0 0 / 0.03), 0px 2px 30px 0px rgb(0 0 0 / 0.08), 0px 0px 1px 0px rgb(0 0 0 / 0.3)" }}
        id="popup-selected"
        className="popup-selected-note min-w-[150px] md:max-w-[270px] absolute border-[0.5px] border-gray bg-[#E8F4FF] z-[1111] rounded-[14px] p-[20px]"
      >
    ...
    <div>
      {loading ? (
        ...
        <div className="w-full content-exam">
          ...
          <HeaderUI part="Listening" title={quiz?.title} />
          <div className="zoomin-content overflow-y-hidden lg:flex flex-row-reverse lg:flex-row  items-center lg:h-[calc(100vh-133px)]">
            <div className="relative lg:w-[50%] h-[calc(100vh-133px)] overflow-y-scroll" id="screen-question">
            ...
            <div className="hidden lg:block w-[50%] bg-neu7 h-full relative">
              <div className="m-auto mt-[100px]">
                ...
                <AnswerUnanser questions={quiz?.parts} part="listening" quiz={quiz} listAnswer={listAnswer} typeChanged={typeChanged} data={sortData} action={action} />
              </div>
            </div>
          </div>
          <FooterUI
